      ...
      setData(prevData => {
        const newMetrics = prevData.metrics.map(metric => {
          ...
          return {
            ...
            history: newHistory,
            change: parseFloat(((newValue - metric.history[0]) / metric.history[0] * 100).toFixed(1)),
        ...
        if (shouldAddActivity) {
          const activityTypes = [
            { type: 'content_published', message: 'New AI content published to Instagram', severity: 'success' },
            { type: 'user_engaged', message: 'User engagement spike detected', severity: 'info' },
            { type: 'ai_generated', message: 'AI model completed content generation', severity: 'success' },
            { type: '3d_rendered', message: '3D visualization render completed', severity: 'info' }
          ] as const;
        ...
        const newPerformance = {
          contentCreation: Math.max(0, Math.min(100, prevData.performance.contentCreation + (Math.random() - 0.5) * 2)),
          userEngagement: Math.max(0, Math.min(100, prevData.performance.userEngagement + (Math.random() - 0.5) * 1.5)),
          aiProcessing: Math.max(0, Math.min(100, prevData.performance.aiProcessing + (Math.random() - 0.5) * 0.5)),
          renderQueue: Math.max(0, Math.min(100, prevData.performance.renderQueue + (Math.random() - 0.5) * 3))
        };
        ...
        if (newPerformance.contentCreation > 90) {
          newInsights.push({
            ...
            description: 'Your content creation rate is at an all-time high! Consider scaling your distribution.',
        ...
        if (newPerformance.renderQueue < 60) {
          newInsights.push({
            ...
            description: 'Low render queue detected. Perfect time to queue more 3D visualizations.',
